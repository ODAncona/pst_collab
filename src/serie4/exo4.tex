\begin{exo}
  \donnee{Un serveur de calcul reçoit des requêtes à fréquence régulière. Elles sont traitées indépendamment les unes après les autres dans leur ordre d’arrivée. On suppose que le nombre de requêtes par minute qui arrivent au serveur peut être modélisé par un processus de Poisson. Les requêtes sont envoyées vers le serveur à un rythme moyen de 6 requêtes par minute.}
